...
interface DiscoveryTrendChartProps {
  metrics: DataMetrics;
  title?: string;
}
...
const DiscoveryTrendChart: React.FC<DiscoveryTrendChartProps> = ({
  metrics,
  title = "Discovery Trend - Published CVEs Over Time"
}) => {
  ...
  const availableSeverities = useMemo(() => {
    if (!metrics.discoveryTrend || Object.keys(metrics.discoveryTrend).length === 0) return [];
    return Array.from(new Set(
      Object.values(metrics.discoveryTrend)
        .flatMap(severities => Object.keys(severities))
    ));
  }, [metrics.discoveryTrend]);
  ...
  // Process discovery trend data
  const chartData = useMemo(() => {
    if (!metrics.discoveryTrend || Object.keys(metrics.discoveryTrend).length === 0) return [];

    // Convert month -> severity -> count mapping to chart data format
    const processedData = Object.entries(metrics.discoveryTrend)
  ...
  }, [metrics.discoveryTrend, zoomRange, availableSeverities]);
      ...
      <Card>
        <CardContent>
          ...
          <Typography color="text.secondary">
            Metrics not available for discovery trend analysis
          </Typography>
        </CardContent>
      </Card>
    );
      ...
      <CardContent>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
          <Typography variant="h6" sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            Discovery Trend - Published CVEs Over Time
          </Typography>
        ...
        <Box sx={{ mb: 3, p: 2, backgroundColor: theme.palette.grey[50], borderRadius: 1 }}>
          <Typography variant="body2" color="text.secondary">
            Shows number of vulnerabilities published each month.
          </Typography>
        </Box>
          ...
          <ResponsiveContainer width="100%" height="100%">
            {chartType === 'line' ? (
              <LineChart data={chartData}>
                ...
                <YAxis 
                  tick={{ fontSize: 12 }}
                  label={{ value: 'CVEs Published', angle: -90, position: 'insideLeft' }}
                />
